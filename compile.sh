...
# Define variables
PROJECT_DIR=$(pwd)
MAIN_CLASS=mymod.TestMod
JAR_NAME=Mod.jar
MANIFEST_FILE=Manifest.txt
EXTERNAL_JAR=~/IdeaProjects/Tomato/Tomato.jar  # Path to the external JAR
...
javac -cp $PROJECT_DIR/src:$EXTERNAL_JAR -d $PROJECT_DIR/out $(find $PROJECT_DIR/src -name '*.java')

# Step 3: Create Manifest file
echo "Creating Manifest file..."
echo "Main-Class: $MAIN_CLASS" > $PROJECT_DIR/$MANIFEST_FILE
echo "Class-Path: $EXTERNAL_JAR" >> $PROJECT_DIR/$MANIFEST_FILE
...
mv $PROJECT_DIR/$JAR_NAME $PROJECT_DIR/mods
cp $EXTERNAL_JAR $PROJECT_DIR/Tomato.jar
java -jar $PROJECT_DIR/Tomato.jar
